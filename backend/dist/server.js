...
async function ensureAdminUser() {
    ...
    const [existingRows] = await db_1.default.query('SELECT UserID FROM Users WHERE Username = :username LIMIT 1', { username: DEFAULT_ADMIN_USERNAME });
    const { hash, salt } = await (0, auth_1.createPasswordRecord)(DEFAULT_ADMIN_PASSWORD);
    if (existingRows.length === 0) {
        await db_1.default.execute(`INSERT INTO Users (Username, PasswordHash, PasswordSalt, Role)
...
async function seedAdminDemoData() {
    await db_1.default.query(ADMIN_SEED_EXERCISES_SQL);
    await db_1.default.query(ADMIN_SEED_SESSIONS_SQL);
    await db_1.default.query(ADMIN_SEED_SESSION_EXERCISES_SQL);
    await db_1.default.query(ADMIN_SEED_OUTCOME_MEASURES_SQL);
}
