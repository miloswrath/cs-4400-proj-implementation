...
import pool, {
  ensureDerivedStructures,
  ensureReferralsConstraint,
...
type TherapistUpcomingSessionRow = RowDataPacket & {
  SessionID: number;
  SessionDate: string;
  SessionTime: string;
  Status: 'Scheduled' | 'Completed' | 'Canceled' | 'No-Show';
...
type PatientHistoryRow = RowDataPacket & {
  SessionID: number;
  PatientID: number;
  SessionDate: string;
  SessionTime: string;
...
type OutcomeProgressRow = RowDataPacket & {
  PatientID: number;
  MeasureName: string;
  BaselineScore: number | null;
...
type StartSessionBody = {
  ...
  outcomeMeasures?: OutcomeMeasureInput[];
};
...
function generateTempPassword(length = 12): string {
  const raw = crypto.randomBytes(length).toString('base64').replace(/[^a-zA-Z0-9]/g, '');
...
app.post('/auth/change-password', async (req, res) => {
  ...
  try {
    ...
    const record = rows[0]!;
    const isValid = await verifyPassword(currentPassword, record.PasswordHash, record.PasswordSalt);
    ...
    await pool.execute(
      `UPDATE Users
       ...
       WHERE UserID = :userId`,
      { hash, salt, userId: numericUserId },
    );
...
app.get('/exercises', async (_req, res) => {
  try {
    const [rows] = await pool.query<ExerciseRow[]>(`SELECT ExerciseID, Name, BodyRegion, Difficulty FROM Exercises ORDER BY Name ASC`);
    res.json({
...
app.post('/admin/therapists', async (req, res) => {
  ...
  if (!trimmedName) errors.push('Therapist name is required.');
  if (!trimmedPhone) errors.push('Phone number is required.');
  if (!trimmedSpecialty) errors.push('Specialty is required.');
  if (!normalizedUsername) errors.push('Username is required.');
  if (!dobValue || Number.isNaN(Date.parse(dobValue))) errors.push('A valid date of birth is required.');
  ...
  try {
    ...
    await connection.execute<ResultSetHeader>(
      ...
      { staffId, specialty: trimmedSpecialty },
    );

    const tempPassword = generateTempPassword();
    const { hash, salt } = await createPasswordRecord(tempPassword);
...
app.get('/therapists/:therapistId/dashboard', async (req, res) => {
  ...
  try {
    const [therapistRows] = await pool.query<RowDataPacket[]>(
    ...
    const [upcomingRows] = await pool.query<TherapistUpcomingSessionRow[]>(
      `SELECT SessionID,
              SessionDate,
              SessionTime,
              Status,
    ...
    const buildInClause = (values: number[], prefix: string) => {
    ...
    const [historyRows] = await pool.query<PatientHistoryRow[]>(
      `WITH ranked AS (
      ...
      { therapistId, today: todayString, ...historyParams },
    );
    ...
    const [outcomeRows] = await pool.query<OutcomeProgressRow[]>(
      `WITH ranked AS (
...
app.post('/therapists/:therapistId/sessions/:sessionId/start', async (req, res) => {
  ...
  if (!SESSION_STATUSES.has(status as any)) {
    res.status(400).json({ message: 'A valid session status is required.' });
    return;
  }

  const normalizePain = (value: unknown, fallback: number | null): number | null => {
  ...
  const sanitizedExercises: SessionExerciseInput[] = Array.isArray(body.sessionExercises)
    ? body.sessionExercises
        ...
        .filter(
          (entry) =>
            Number.isInteger(entry.exerciseId) &&
            entry.exerciseId > 0 &&
  ...
  const sanitizedOutcomes: OutcomeMeasureInput[] = Array.isArray(body.outcomeMeasures)
  ...
  try {
    ...
    const [sessionRows] = await connection.query<RowDataPacket[]>(
      `SELECT SessionID, PatientID, PainPre, PainPost, Notes
       FROM Sessions
    ...
    await connection.execute<ResultSetHeader>('DELETE FROM SessionExercises WHERE SessionID = :sessionId', {
      sessionId,
    });
...
app.get('/patients/:patientId/sessions', async (req, res) => {
  ...
  try {
    const [upcomingRows] = await pool.query<PatientSessionRow[]>(
      `SELECT SessionID,
              SessionDate,
              SessionTime,
              Status,
    ...
    const [pastRows] = await pool.query<PatientSessionRow[]>(
      `SELECT SessionID,
              SessionDate,
              SessionTime,
              Status,
